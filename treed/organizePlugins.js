...
const getSubThing = (
  ...
  sub: string,
  thing: string,
) => plugins.filter(p => p[sub] && p[sub][thing]).map(p => p[sub][thing])

const getPluginThing = (
  plugins: Array<Plugin<*, *>>,
  sub: string,
  thing: string,
) =>
  plugins
    ...
    .map(p => (node, store) => p[sub][thing](node.plugins[p.id], node, store))
...
const typesContextMenu = (node, store) => {
  ...
  const children = Object.keys(store.plugins.nodeTypes)
    .filter(key => !store.plugins.nodeTypes[key].disableSwitch)
    .map(key => {
...
const organizePlugins = (
  pluginSettings: {[pluginId: string]: any},
  plugins: {[pluginId: string]: Plugin<*, *>},
): PluginSummary => {
  const enabledPlugins = Object.keys(pluginSettings)
    .map(id => plugins[id])
  ...
  enabledPlugins.forEach(plugin => {
    ...
    Object.keys(pnodeTypes).forEach(type => {
      if (nodeTypes[type]) {
        console.error(
          `Multiple plugins want to own ${type}: ${plugin.id} is overriding`,
        )
      }
  ...
  return {
    nodeTypes,
    node: {
      className: classNameGetters.length === 1
        ? classNameGetters[0]
        : classNameGetters.length === 0
            ? null
            : (node, store) =>
                classNameGetters.map(f => f(node, store)).join(' '),
    ...
    actionButtons: enabledPlugins
      .filter(p => p.actionButtons)
      .map(p => ({id: p.id, buttons: p.actionButtons})),
  }
}
